...
플레이 화면은 아이콘과 이름을 공유하는 마법 타일, 두 열의 `지금 뜨면 고르세요`, 접을 수 있는 `내 조합`, 두 재료를 나란히 보여주는 조합 카드로 구성합니다. 필요한 마법 옆의 +1로 바로 기록할 수 있으며, 기록은 보고 있던 조합을 바꾸지 않습니다. 마법 검색과 전체/찍은 마법/목표 재료 필터는 기록을 변경하지 않습니다. 목표 이름은 재료를 펼치며 해제는 별도 버튼입니다.

특성 선택창에는 선택지와 목표에 필요한 특성을 먼저 표시합니다. 연결 조합·효과·이미 사용한 재료 정보는 각 선택지 아래에서 펼칩니다. 조합 도감은 검색과 가능한 조합/전체/내 목표 필터, 동일한 재료 카드를 사용합니다. 도감의 가능 여부는 기존 판정 결과이며 전투 성능 순위가 아닙니다. 아이콘은 탐색용 심볼이며 게임 원본 이미지가 아닙니다.
