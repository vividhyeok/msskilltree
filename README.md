...
- **Run 설정**: 목표·빌드·시간대와 직접 확인한 게임 버전을 입력하면 상황별 추천을 표시합니다. 클래스·실험체·궁극기, 보유 유물/완성 시너지, 남은 마법 선택 횟수는 선택 입력입니다.
- **유물 선택**: 게임에 뜬 3~4개를 비교하고 실제 선택한 것만 기록합니다. 시너지 완성 근접도는 게임에서 확인한 재료 상태를 입력한 경우에만 반영합니다.
- 상황별 추천은 커뮤니티 의견으로 표시하고 이유·출처·신뢰도·의견 차이를 함께 제공합니다. 기존 최소 추가 레벨 추천은 **성장 효율 경로**로 분리했습니다.
- HTTPS에서 한 번 방문 후 오프라인 사용 가능. 브라우저 메뉴에서 홈 화면에 설치할 수 있습니다.

1024×768에서는 모든 액티브와 패시브가 화면 안에 있고 body 스크롤이 없습니다. 최소 타일 너비에 따라 열 수를 조정하고, 모바일은 3열과 목표/경로 bottom sheet를 사용합니다. 새 Run은 고정 가나다순이며 이전 저장의 찍은 마법 우선 정렬은 유지합니다. Run 설정에서 배치를 선택할 수 있습니다.
...
- `src/meta/`: 기존 엔진을 먼저 확인하는 별도 커뮤니티 메타 평가·설정·유물 비교
- `src-data/community-meta/current/`: 실행 중 사용하는 패치별 메타 규칙, 출처, 용어/특성 연결

메타 seed의 범위, v0.993 이후 데이터 갱신 방법과 QA는 [Community Meta 구현 안내](src-data/community-meta/IMPLEMENTATION.md)를 참고하세요. `current/`의 데이터만 바꿔 추천을 갱신할 수 있습니다. 제공된 research/data 원본은 같은 폴더에 snapshot으로 보존했습니다.
...
NEED NOW는 목표의 generic requirements를 순회하며 마법 ID, 필요 레벨, 특성 단계·특성·참여 역할로 묶습니다. 공유 재료의 배지는 합치되 역할이 다르거나 막힌 목표와 가능한 목표는 분리합니다. 완료 후 승계 원본과 병합 재료를 구분해 표시하며 둘 다 원본 재사용은 잠급니다. 마력탄 4·7단계, 패시브 최소 레벨과 완료 선행 조건을 유지합니다.
