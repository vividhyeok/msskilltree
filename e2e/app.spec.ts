...
function layoutSnapshot(page: Page) {
  return page.locator(".magic-tile").evaluateAll((els) =>
    els.map((el) => ({
...
test("saved build glance, inspect versus record, traits, completion, locks, undo and restore", async ({
  ...
  const need = page.getByRole("region", {
    name: "현재 목표에 필요한 선택",
  });
  ...
  await expect(need).toContainText(/승계|병합/);
  ...
  await expect(energy.locator(".need-role")).toHaveText("병합");
  await expect(energy.locator(".need-inspect")).toHaveAttribute(
    "title",
    /재료 마법/,
  );
  ...
  await expect(
    page.getByRole("region", { name: "선택 상세와 전략" }),
  ).toContainText("청천벽력");
  ...
  expect(await page.locator(".target-section").boundingBox()).toEqual(targetBox);
  ...
  // Investment no longer reorders the live board; muscle-memory positions stay fixed.
  ...
  const strategy = page.locator(".strategy-drawer");
  await expect(strategy).not.toHaveAttribute("open", "");
...
test("one-tap path pinning and conflicting badges", async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(tile(page, "energy_bolt").locator(".badge")).toHaveText(["A", "B"]);
...
for (const [width, height] of [
  ...
  test(`HUD fits ${width}x${height}`, async ({ page }) => {
    ...
    if (width > 650) {
      ...
      for (const el of await page.locator(".tile-add,.tile-inspect,.passive-add").all()) {
    ...
    await page.screenshot({ path: `test-results/hud-${width}.png`, fullPage: true });
  });
...
test("three targets and six ingredients stay visible at 1024x768", async ({
  ...
  for (const el of await page.locator(".target-row,.need-row").all())
    await expect(el).toBeInViewport({ ratio: 1 });
  expect(
    await page.locator(".target-section").evaluate((el) => el.scrollHeight <= el.clientHeight),
  ).toBe(true);
  expect(
    await page.locator(".need-section").evaluate((el) => el.scrollHeight <= el.clientHeight),
  ).toBe(true);
